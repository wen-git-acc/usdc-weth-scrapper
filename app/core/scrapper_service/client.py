...
class ScrapperService:
    ...
    def get_decode_uniswap_v3_executed_price(self, tx_hash: str, contract_address: str) -> list[TransactionSwapExecutionPrice]:
        ...
        for log in receipt.logs:
            if log["address"].lower() == contract_address and log["topics"][0].hex() == event_signature:  # Replace with actual Uniswap contract address
                try:
                    event_data = self.__web3py.eth.contract(abi=uniswap_v3_swap_abi).events.Swap().process_log(log)
                    amount0 = event_data['args']['amount0']
                    amount1 = event_data['args']['amount1']
                    sqrtPriceX96 = event_data['args']['sqrtPriceX96']

                    (token_received, token_sent) = self.getTokenDetails(amount0, amount1)
    ...
    def getTokenDetails(self, amount0: int, amount1: int) -> Tuple[TokenDetail, TokenDetail]:
    ...
    def calculate_execution_price(self, sqrtPriceX96):
        # Using decimal for precision in large numbers
        sqrt_price_decimal = Decimal(sqrtPriceX96)
        
        # Price calculation
        price = (sqrt_price_decimal ** 2) / (2 ** 192)
        return float(price)
